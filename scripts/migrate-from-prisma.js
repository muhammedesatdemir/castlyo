...
async function migratePrismaToNew() {
  const sql = postgres(DATABASE_URL, { max: 1 });

  try {
    console.log('🔄 Starting migration from Prisma to new schema...');
    ...
    try {
      ...
      const profiles = await sql`SELECT COUNT(*) as count FROM "Profile"`;
    ...
    // Create users table (migrating from User)
    await sql.unsafe(`
    ...
    await sql.unsafe(`
      ...
      SELECT 
        ...
        password, 
        CASE 
          WHEN role = 'TALENT' THEN 'TALENT'::user_role
          WHEN role = 'AGENCY' THEN 'AGENCY'::user_role
          ELSE 'TALENT'::user_role
        END,
        'ACTIVE'::user_status,
    ...
    // Create talent_profiles table (migrating from Profile)
    await sql.unsafe(`
    ...
    // Migrate data from Profile to talent_profiles
    console.log('🔄 Migrating profiles...');
    await sql.unsafe(`
      INSERT INTO talent_profiles (
        id, user_id, first_name, last_name, bio, phone, city, 
        experience, height_cm, weight_kg, specialties, languages, 
    ...
    console.log('🎉 Migration from Prisma to new schema completed!');
    
    // Show final status
    const finalUsers = await sql`SELECT COUNT(*) as count FROM users`;
    const finalProfiles = await sql`SELECT COUNT(*) as count FROM talent_profiles`;
    console.log(`\n📊 Final counts:`);
    console.log(`  - Users: ${finalUsers[0].count}`);
    console.log(`  - Talent Profiles: ${finalProfiles[0].count}`);
...
migratePrismaToNew().catch(console.error);
